...
{
	year  = $('#windradii_year_selector').val()
	...
	{
	    result = data['result']
	    h = '<option name=windradii_year_option value="ignore">Select</option>'
	    for (var s in result)
	    {
		year = result[s];
		h = h + '<option name=windradii_year_option value="' + year + '">' + year + '</option>';
	    }
	    $('#windradii_year_selector').html(h);
	}
}
...
function receive_storms(data)
{
	...
	{
	    result = data['result']
	    h = '<option name=windradii_storm_option value="ignore">Select</option>'
	    for (var s in result)
	    {
		storm = result[s];
		h = h + '<option name=windradii_storm_option value="' + storm + '">' + storm + '</option>';
	    }
	    $('#windradii_storm_selector').html(h);
	    $('#windradii_storm_selection').show();
	}
}
...
{
	year  = $('#windradii_year_selector').val()
	var u = '/windradii/get_storms/' + year;
...
{
	year  = $('#windradii_year_selector').val()
	storm = $('#windradii_storm_selector').val()
	var u = '/windradii/get_storm_contours/' + year + '/' + storm;
...
$(document).ready(function() {
	...
	eps4326 = new OpenLayers.Projection("EPSG:4326");
 	...
 	$.ajax({url: '/windradii/get_years', dataType: 'json', cache: false, success: receive_years});
